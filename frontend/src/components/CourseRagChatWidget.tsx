...
function formatBotReply(res: {
  ...
  // Only show document-grounded text — never blend in free-form AI extras.
  return res.used_document ? `From your documents:\n\n${doc}` : doc;
}
...
export function CourseRagChatWidget({ classroomId }: { classroomId: number }) {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      const res = await aiApi.chat({ classroom_id: classroomId, question });
      setMessages((prev) => [
        ...prev,
        { id: `b-${Date.now()}`, role: "bot", text: formatBotReply(res) },
      ]);
    } catch (err) {
      const message = err instanceof Error ? err.message : "Could not reach the course assistant.";
      setError(message);
      setMessages((prev) => [
        ...prev,
        {
          ...
          <div className="border-b border-[#eef1f4] px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#75777f]">Document-grounded help</p>
              </div>
            </div>
          </div>
