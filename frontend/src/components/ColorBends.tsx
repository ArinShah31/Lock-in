...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < MAX_COLORS; i++) {
      if (i < initialColors.length) uColorsArray[i].copy(initialColors[i]);
      else uColorsArray[i].set(0, 0, 0);
    }
    material.uniforms.uColorCount.value = initialColors.length;
  ...
  return (
    <div
      ref={containerRef}
      className={`relative h-full w-full min-h-full overflow-hidden ${className}`}
      style={style}
    />
  );
}
