...
from app.services.theory_platform_sync import SYNCABLE_ROLES, sync_user_to_theory_platform

router = APIRouter(prefix="/theory-platform", tags=["theory-platform"])


class TheoryAccessOut(BaseModel):
    enabled: bool
...
class TheoryStudentOut(BaseModel):
    id: int
    full_name: str
    email: str


def _student_has_theory_access(db: Session, student: User) -> bool:
    memberships = (
    ...
    enabled_count = (
        ...
        .filter(
            ...
            User.theory_platform_enabled.is_(True),
...
def user_has_theory_access(db: Session, user: User) -> tuple[bool, str | None]:
    if user.role in {UserRole.CLASS_TEACHER, UserRole.SUBJECT_TEACHER}:
        if user.theory_platform_enabled:
            return True, None
        return False, "Ask your HOD to enable the theory platform."

    if user.role == UserRole.STUDENT:
        if _student_has_theory_access(db, user):
            return True, None
        return False, "Ask your HOD to enable the theory platform for your teachers."

    return False, "Theory platform is not available for your role."


@router.get("/access", response_model=TheoryAccessOut)
def theory_access(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    enabled, reason = user_has_theory_access(db, current_user)
    return TheoryAccessOut(
        enabled=enabled,
        reason=reason,
        frontend_url=settings.theory_platform_frontend_url.rstrip("/"),
    )


@router.get("/students", response_model=list[TheoryStudentOut])
def list_theory_students(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    for student in rows:
        ...
        out.append(
            TheoryStudentOut(id=student.id, full_name=student.full_name, email=student.email)
        )
    return out
...
def issue_sso_token(
    ...
    if not enabled:
        raise HTTPException(
            ...
            detail=reason or "Theory platform is not enabled",
        )
    if current_user.role not in SYNCABLE_ROLES:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Role cannot use theory platform")

    synced = sync_user_to_theory_platform(current_user)
    if not synced:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=(
                "Could not sync your account to the theory platform. "
                "Ensure the theory API is running and THEORY_PLATFORM_API_URL / THEORY_SYNC_SECRET match."
            ),
        )
