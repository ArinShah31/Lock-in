...
def create_user(
    ...
    # TEMPORARY:
    # Allow Super Admin to create any user while frontend is being built.
    if current_user.role == UserRole.SUPER_ADMIN:
        ...
        if not institution or not institution.is_active:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid or inactive institution",
            )
        ...
        if department.institution_id != current_user.institution_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Department not in your institution",
            )
...
@router.patch("/{user_id}/coding-platform", response_model=UserOut)
def set_coding_platform(
    user_id: int,
    payload: CodingPlatformToggleRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    if current_user.role != UserRole.HOD:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only HOD can enable or disable the coding platform for teachers",
        )
    ...
    if target.role not in TEACHER_ROLES:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Coding platform can only be toggled for class or subject teachers",
        )
    ...
    target.coding_platform_enabled = payload.enabled
    db.commit()
    db.refresh(target)
    if payload.enabled:
        sync_user_to_coding_platform(target)
        # Ensure classroom students exist on the coding platform for assign/SSO.
...
def list_users(
    ...
    if current_user.role == UserRole.INSTITUTION_ADMIN:
        if not current_user.institution_id:
            return []
        return (
            query.filter(User.institution_id == current_user.institution_id)
            .order_by(User.id)
            .all()
        )
    ...
    if current_user.role == UserRole.HOD:
        if not current_user.department_id:
            return []
        return (
            query.filter(User.department_id == current_user.department_id)
            .order_by(User.id)
            .all()
        )
