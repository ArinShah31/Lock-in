...
def _finalize_session(
    ...
    *,
    ...
    test = assignment.theory_test
    links = (
        db.query(TheoryTestQuestion)
        .filter(TheoryTestQuestion.theory_test_id == test.id)
        .order_by(TheoryTestQuestion.order_index)
        .all()
    )
    for link in links:
        draft = (
            db.query(AnswerDraft)
            .filter(AnswerDraft.session_id == session.id, AnswerDraft.question_id == link.question_id)
            .first()
        )
        answer_text = draft.answer_text if draft else ""
        existing = (
            db.query(AnswerSubmission)
            .filter(AnswerSubmission.session_id == session.id, AnswerSubmission.question_id == link.question_id)
            .first()
        )
        if not existing:
            existing = AnswerSubmission(
            ...
            db.add(existing)
            db.flush()
        elif existing.answer_text != answer_text:
...
def join_by_invite(
    ...
    test = (
        db.query(TheoryTest)
        .filter(TheoryTest.invite_code == payload.invite_code.upper().strip(), TheoryTest.is_active.is_(True))
        .first()
    )
    if not test:
    ...
    row = TestAssignment(theory_test_id=test.id, student_id=student.id, status=AssignmentStatus.ASSIGNED)
    db.add(row)
    db.commit()
    db.refresh(row)
    return AssignmentOut(
...
def my_assignments(db: Session = Depends(get_db), student: User = Depends(require_student)):
    ...
    for row in rows:
        test = db.query(TheoryTest).filter(TheoryTest.id == row.theory_test_id).first()
        out.append(
            AssignmentOut(
                id=row.id,
                theory_test_id=row.theory_test_id,
                student_id=student.id,
                status=row.status,
...
def exam_questions(
    ...
    links = (
        db.query(TheoryTestQuestion)
        .filter(TheoryTestQuestion.theory_test_id == session.assignment.theory_test_id)
        .order_by(TheoryTestQuestion.order_index)
        .all()
    )
    ...
    for link in links:
        ...
        out.append(
            ExamQuestionOut(
                ...
                draft_answer=draft.answer_text if draft else None,
            )
        )
    return out
...
def save_draft(
    ...
    link = (
        db.query(TheoryTestQuestion)
        .filter(
            TheoryTestQuestion.theory_test_id == session.assignment.theory_test_id,
            TheoryTestQuestion.question_id == payload.question_id,
        )
        .first()
    )
    if not link:
    ...
    draft = (
        db.query(AnswerDraft)
        .filter(AnswerDraft.session_id == session.id, AnswerDraft.question_id == payload.question_id)
        .first()
    )
    if not draft:
        draft = AnswerDraft(
    ...
    max_order = (
        db.query(func.max(TheoryTestQuestion.order_index))
        .filter(TheoryTestQuestion.theory_test_id == session.assignment.theory_test_id)
        .scalar()
    ) or 1
...
def _session_evals(db: Session, session: TestSession) -> list[EvalOut]:
    subs = db.query(AnswerSubmission).filter(AnswerSubmission.session_id == session.id).all()
    out: list[EvalOut] = []
    for sub in subs:
...
def teacher_attempts(
    ...
    test = db.query(TheoryTest).filter(TheoryTest.id == test_id, TheoryTest.created_by_id == teacher.id).first()
    if not test:
        raise HTTPException(status_code=404, detail="Test not found")
    assignments = db.query(TestAssignment).filter(TestAssignment.theory_test_id == test.id).all()
...
@results_router.get("/analytics", response_model=TeacherTheoryAnalyticsOut)
def teacher_coding_analytics(
    ...
    empty = TeacherTheoryAnalyticsOut(
    ...
    query = (
        db.query(TestAssignment, TheoryTest, User)
        .join(TheoryTest, TheoryTest.id == TestAssignment.theory_test_id)
        .join(User, User.id == TestAssignment.student_id)
        .filter(TestAssignment.theory_test_id.in_(test_ids))
    )
    if student_id is not None:
        query = query.filter(TestAssignment.student_id == student_id)
    rows = query.order_by(TheoryTest.id.asc()).all()
    ...
    return TeacherTheoryAnalyticsOut(
...
def teacher_result_students(
    ...
    rows = (
        db.query(TestAssignment, User, TheoryTest)
        .join(TheoryTest, TheoryTest.id == TestAssignment.theory_test_id)
        .join(User, User.id == TestAssignment.student_id)
        .filter(TheoryTest.created_by_id == teacher.id)
        .all()
    )
...
def teacher_student_attempts(
    ...
    rows = (
        db.query(TestAssignment, TheoryTest)
        .join(TheoryTest, TheoryTest.id == TestAssignment.theory_test_id)
        .filter(
            TheoryTest.created_by_id == teacher.id,
            TestAssignment.student_id == student_id,
        )
        .order_by(TheoryTest.id.asc())
        .all()
    )
    if not rows:
        student = db.query(User).filter(User.id == student_id).first()
...
def teacher_update_eval(
    ...
    sub = db.query(AnswerSubmission).filter(AnswerSubmission.id == ev.submission_id).first()
    if not sub:
        raise HTTPException(status_code=404, detail="Submission not found")
    session = db.query(TestSession).filter(TestSession.id == sub.session_id).first()
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    test = session.assignment.theory_test
